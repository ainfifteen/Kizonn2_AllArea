...
public class Main{
	public static void main(String args[]) throws IOException{
		...
		int falsePeaple = normalPeaple * 2;

		//long sleepTime = (long) (simTime * 1000);
		Random rnd = new Random();
		int[]iValue = new int[normalPeaple];
		int[]jValue = new int[normalPeaple];
		...
		while(lapseTime < endTime) {
			...
			for(int i = 0; i < 9; i++) {
				...
				edgeServer.receiveData(area[i][0], area[i][1], lapseTime);
			...
			System.out.println(falsePeaple);
				if(falsePeaple == 0) {
					...
					falsePeaple = 10000;
					try {//ファイルへの書き込み
						FileWriter fw = new FileWriter("/Users/sachi/Desktop/detectPeapleTime.txt",true);
						BufferedWriter bw = new BufferedWriter(fw);
						bw.write(str);
						bw.newLine();//改行
						bw.flush();
